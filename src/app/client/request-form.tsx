    ...
    <form action={submitClientRequest} className="space-y-5">
      <section className="overflow-hidden rounded-[28px] bg-white shadow-[0_14px_40px_rgba(15,23,42,.08)] ring-1 ring-black/5">
        ...
        <div className="grid gap-4 p-5 sm:grid-cols-2">
          <label className={labelClass}>依頼会社名<input name="companyName" required className={inputClass} placeholder="株式会社〇〇" /></label>
      ...
      <section className="overflow-hidden rounded-[28px] bg-white shadow-[0_14px_40px_rgba(15,23,42,.08)] ring-1 ring-black/5">
        ...
        <div className="space-y-4 p-5">
          {isBand ? <div className="grid gap-4 sm:grid-cols-2"><label className={labelClass}>対象月<input name="yearMonth" type="month" required className={inputClass}/></label><label className={labelClass}>予定稼働日数<input name="plannedDays" type="number" min="1" required className={inputClass} placeholder="20"/></label></div> : <div className="rounded-2xl bg-[#f3f6f8] p-4 text-sm font-bold text-slate-600">スポット依頼は、実際に稼働する日を下で入力してください。</div>}
          ...
          {schedulePattern === "FIXED" && <div className="grid gap-4 sm:grid-cols-2"><label className={labelClass}>開始時間<input name="fixedStartTime" type="time" required className={inputClass}/></label><label className={labelClass}>終了時間<input name="fixedEndTime" type="time" required className={inputClass}/></label></div>}
          {!isBand && <div><div className="mb-2 flex items-center justify-between"><p className="text-[13px] font-black text-slate-700">スポット稼働日</p><button type="button" onClick={()=>setDays([...days,{date:"",start:"10:00",end:"19:00"}])} className="flex items-center gap-1 rounded-xl bg-[#eef2f5] px-3 py-2 text-xs font-black text-[#14283b]"><Plus size={14}/>日付追加</button></div><div className="space-y-2">{days.map((day,i)=><div key={i} className={`grid gap-2 ${schedulePattern==="VARIES"?"sm:grid-cols-[1fr_1fr_1fr_auto]":"sm:grid-cols-[1fr_auto]"}`}><input name="scheduleDate" type="date" required className={`${inputClass} mt-0`}/>{schedulePattern==="VARIES"&&<><input name="scheduleStart" type="time" required defaultValue={day.start} className={`${inputClass} mt-0`}/><input name="scheduleEnd" type="time" required defaultValue={day.end} className={`${inputClass} mt-0`}/></>}<button type="button" disabled={days.length===1} onClick={()=>setDays(days.filter((_,x)=>x!==i))} className="grid min-h-12 w-12 place-items-center rounded-2xl border border-slate-200 text-slate-400 disabled:opacity-30"><Trash2 size={17}/></button></div>)}</div></div>}
        </div>
      </section>
      ...
      <section className="overflow-hidden rounded-[28px] bg-white shadow-[0_14px_40px_rgba(15,23,42,.08)] ring-1 ring-black/5">
        ...
        <div className="grid gap-4 p-5 sm:grid-cols-2">
          ...
          <label className={labelClass}>単価（税抜）<input name="rateAmountExTax" type="number" min="0" required className={inputClass} placeholder={contractType==="MONTHLY"?"600000":"30000"}/></label>
